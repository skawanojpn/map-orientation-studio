...
const regionLabelsJa: Record<string, string> = { Japan: "日本", "East Asia": "東アジア", "Southeast Asia": "東南アジア", "South Asia": "南アジア", Oceania: "オセアニア", Europe: "ヨーロッパ", Americas: "南北アメリカ", "South America": "南アメリカ", Africa: "アフリカ", "Middle East": "中東", Special: "特殊地点" };
function cityName(city: Point) { return state.lang === "ja" ? cityLabelsJa[city.name] || city.name : city.name; }
function cityRegion(city: Point) { return cityRegions[city.name] || "Other"; }
...
function render() {
  ...
  if (state.layers.routes) { const route = { type: "LineString", coordinates: Array.from({ length: 101 }, (_, i) => d3.geoInterpolate([state.lng, state.lat], compareCoord)(i / 100)) }; context.beginPath(); path(route as any); context.strokeStyle = t.accent; context.lineWidth = 2; context.setLineDash([8, 5]); context.stroke(); context.setLineDash([]); }
  for (const [coord, color, label] of [[[state.lng, state.lat], "#ff4d6d", "A"], [compareCoord, "#4dabf7", "B"]] as [[number, number], string, string][]) { const p = projection(coord); if (p) { context.beginPath(); context.arc(p[0], p[1], 5, 0, Math.PI * 2); context.fillStyle = color; context.fill(); context.fillStyle = t.bg; context.font = "bold 8px sans-serif"; context.textAlign = "center"; context.textBaseline = "middle"; context.fillText(label, p[0], p[1]); context.textAlign = "start"; context.textBaseline = "alphabetic"; } }
...
function refreshCityOptions() { const query = cityQuery.trim().toLocaleLowerCase(); const visibleCities = cities.filter(city => (cityMode === "all" || isMajorCity(city)) && (!query || `${city.name} ${cityName(city)} ${cityRegion(city)} ${cityRegionLabel(cityRegion(city))}`.toLocaleLowerCase().includes(query))); for (const id of ["city", "city-compare"]) { const select = $(id) as HTMLSelectElement; const selected = id === "city-compare" ? JSON.stringify(compareCoord) : select.value; select.replaceChildren(); const groups = new Map<string, HTMLOptGroupElement>(); for (const city of visibleCities) { const region = cityRegion(city); if (!groups.has(region)) { const group = document.createElement("optgroup"); group.label = cityRegionLabel(region); groups.set(region, group); select.appendChild(group); } groups.get(region)!.appendChild(new Option(cityName(city), JSON.stringify(city.coord))); } if ([...select.options].some(option => option.value === selected)) select.value = selected; } }
...
function buildUI() {
  ...
  ["a-lng", "a-lat", "b-lng", "b-lat"].forEach(id => $(id).addEventListener("change", () => { const aLng = Number($(HTMLInputElement, "a-lng").value), aLat = Number($(HTMLInputElement, "a-lat").value), bLng = Number($(HTMLInputElement, "b-lng").value), bLat = Number($(HTMLInputElement, "b-lat").value); if ([aLng, aLat].every(Number.isFinite)) { state.lng = aLng; state.lat = aLat; } if ([bLng, bLat].every(Number.isFinite)) compareCoord = [bLng, bLat]; updateProjection(); render(); }));
  $("city-search").addEventListener("input", e => { cityQuery = (e.target as HTMLInputElement).value; refreshCityOptions(); });
  $("city-mode").addEventListener("change", e => { cityMode = (e.target as HTMLSelectElement).value as "major" | "all"; refreshCityOptions(); render(); });
